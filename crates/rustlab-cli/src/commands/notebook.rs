...
pub enum NotebookCommands {
    ...
    #[command(
        long_about = "Strip rustlab-generated artifacts from one or more .md files, leaving only \
            ...
            output.\n\n\
            What gets stripped:\n  \
            - <!-- Generated by rustlab-notebook --> header lines\n  \
            - <!-- rustlab:output-start/end --> sentinel regions and their wrapped content\n  \
            - Legacy unwrapped <iframe> tags matching rustlab's exact emission signature\n  \
            - Legacy bare ```text blocks that immediately follow a ```rustlab fence\n\n\
            What's preserved: prose, code fences, hand-authored iframes with different attributes, \
            standalone ```text blocks not adjacent to rustlab fences.\n\n\
            Examples:\n  \
...
pub struct CleanArgs {
    ...
    /// For a directory input, must also be a directory.
    #[arg(short, long)]
    output: Option<PathBuf>,
    /// Report what would change without writing anything. Exits 1
    /// if any file would change, 0 if everything is already clean.
    #[arg(long)]
    check: bool,
}
...
pub struct RenderArgs {
    ...
    /// gains `tags: [rustlab]` and `cssclasses: [rustlab-notebook]`, and a
    /// trailing iframe to the sibling .html is appended (suppress with
    /// `--no-iframe`). Only meaningful with --format markdown; ignored
    /// otherwise.
    #[arg(long)]
    obsidian: bool,
    /// Override the attachments directory used by `--obsidian` for plot
    /// SVGs. Default: `_attachments`. Path is relative to the notebook
    /// output directory.
    #[arg(long, value_name = "DIR")]
    attachments_dir: Option<String>,
    /// Suppress the trailing iframe under `--obsidian`. Useful when you
    /// want the wikilink + attachments rewrites without embedding the
...
pub fn execute(cmd: NotebookCommands) -> Result<()> {
    match cmd {
        NotebookCommands::Watch(args) => {
            let theme = match args.theme {
                CliTheme::Dark => Theme::Dark,
                CliTheme::Light => Theme::Light,
            };
            let colors = theme.colors();
            let obsidian_opts = if args.obsidian {
            ...
            rustlab_notebook::watch::cmd_watch(
                args.input,
                args.output,
                format,
                colors,
        ...
        NotebookCommands::Render(args) => {
            ...
            if (args.attachments_dir.is_some() || args.no_iframe) && !args.obsidian {
                eprintln!(
            ...
            let obsidian_opts = if args.obsidian {
                let mut opts = rustlab_notebook::ObsidianOpts::default();
                if let Some(dir) = args.attachments_dir.clone() {
                    opts.attachments_dir = dir;
                }
                if args.no_iframe {
                    opts.iframe = false;
