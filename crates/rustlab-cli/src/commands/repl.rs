...
const HELP: &[HelpEntry] = &[
    ...
    HelpEntry { name: "spsolve", brief: "Solve sparse linear system  A*x = b",
        detail: "spsolve(A, b)\nspsolve(A, b, mode)\n  A    — sparse or dense square matrix\n  b    — right-hand side vector\n  mode — \"auto\" (default), \"cholesky\", or \"lu\"\n\nDispatch:\n  \"auto\"     — detect Hermitian-positive-definite structure; use the\n              hand-rolled sparse Cholesky if SPD, otherwise dense LU.\n  \"cholesky\" — force the sparse Cholesky path; errors if A is not SPD.\n  \"lu\"       — force the dense LU path (sparse LU lands in a later phase).\n\nThe Cholesky path is the scaling fix for SPD assemblies: Lessons 05-09\n(electrostatics, magnetostatics on uniform grids) all build SPD\nLaplacians and were previously bottlenecked on the dense fallback.\n\nExamples:\n  x = spsolve(speye(3), [1, 2, 3])\n  L = -1 * laplacian_2d(50, 50);\n  x = spsolve(L, ones(2500, 1));               % auto picks Cholesky\n  x = spsolve(L, ones(2500, 1), \"cholesky\");   % explicit" },
