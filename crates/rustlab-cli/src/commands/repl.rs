...
const HELP: &[HelpEntry] = &[
    ...
    HelpEntry { name: "sort",   brief: "Sort vector by real part",
        detail: "sort(v)              — ascending order (default)\nsort(v, \"ascend\")    — explicit ascending\nsort(v, \"descend\")   — descending order\n\n  Returns a new vector; imaginary components are preserved.\n  Comparison uses the real part only.\n  sort([3,1,2])             → [1, 2, 3]\n  sort([3,1,2], \"descend\")  → [3, 2, 1]" },
    ...
    HelpEntry { name: "find", brief: "Find non-zero element positions",
        detail: "find(v)  — dense vector → vector of 1-based element indices\nfind(M)  — dense matrix → vector of 1-based COLUMN-MAJOR linear indices\nfind(S)  — sparse matrix → tuple [I, J, V] (1-based subscripts + values)\nfind(S)  — sparse vector → tuple [I, V]\n\nDense matrix linear indexing follows the octave/matlab convention:\nelement M(i, j) sits at linear index (j - 1) * nrows + i.\n\nExamples:\n  find([0, 5, 0, -3])         → [2, 4]\n  find([0, 2; 3, 0])          → [2, 3]\n  [I, J, V] = find(speye(3))\n  [I, V] = find(sparsevec([1,3], [10,20], 5))" },
