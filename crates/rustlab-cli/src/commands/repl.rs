...
pub const HELP: &[HelpEntry] = &[
    ...
    HelpEntry { name: "hist", brief: "Histogram — plot and return bin counts",
        detail: "hist(v)        — 10 bins (default)\nhist(v, n)     — n bins\nAssigned form returns a 2×n matrix: row 1 = bin centers, row 2 = counts.\nA bare `hist(v)` line draws the chart and echoes nothing.\n\nAlias: histogram()" },
    ...
    HelpEntry { name: "transpose", brief: "Non-conjugate transpose  (also: A.')",
        detail: "transpose(A)  — transposes rows and cols without conjugating\n  Equivalent to the postfix operator A.'\n\n  Warning: postfix ' CONJUGATES — reshaping complex data with\n  src(:)' silently negates every imaginary part. Use src(:).'\n  (or transpose) when the intent is pure reshaping." },
    ...
    // FFT
    HelpEntry { name: "fft",      brief: "Forward DFT — output length equals input length",
        detail: "fft(v)     — complex spectrum with exactly length(v) bins\nfft(v, n)  — zero-pad or truncate v to n samples first (any positive n)\n\nPower-of-two lengths use the radix-2 fast path; other lengths use the\nBluestein (chirp-z) transform. Build the frequency axis with\nf = fftfreq(length(X), fs) — it always matches." },
    HelpEntry { name: "ifft",     brief: "Inverse DFT (any length)",
        detail: "ifft(V)     — inverse transform with 1/n scaling; any input length\nifft(V, n)  — pad or truncate to n first\n\nifft(fft(x)) reconstructs x exactly (same length)." },
    ...
    HelpEntry { name: "histogram", brief: "Histogram bar chart (alias for hist)",
        detail: "histogram(v) / histogram(v, n_bins)\n  Same builtin as hist(); both names are registered.\n\n  At statement position (bare `histogram(v)` line) nothing is echoed.\n  Assign to get the bins: `h = histogram(v)` returns a 2×n matrix\n  (row 1 = bin centers, row 2 = counts).\n\nExample:\n  histogram(randn(1000), 30)" },
    ...
    HelpEntry { name: "i / j", brief: "Imaginary unit constant  (0 + 1i)",
        detail: "i and j are both pre-defined constants equal to sqrt(-1)\n  Example: z = 3 + j*4   or   z = 3 + i*4\n\n  Numeric literals also take an imaginary suffix: 2j, 1.5i, 3e8j.\n  z = 1 + 2j parses directly, and printed complex values (1+2j)\n  paste back as input. The suffix binds to the literal, so it\n  still works when a variable named i or j shadows the constant." },
    ...
    HelpEntry { name: "int8", brief: "Cast to 8-bit signed integer",
        detail: "int8(x [, \"saturate\"|\"wrap\"])  — range −128..127\n  Reals round half away from zero; out-of-range saturates (default) or\n  wraps. int8(200) → 127; int8(2.5) → 3; int8(200, \"wrap\") → -56." },
    HelpEntry { name: "int16", brief: "Cast to 16-bit signed integer",
        detail: "int16(x [, mode])  — range −32768..32767. See int8 for cast rules." },
    HelpEntry { name: "int32", brief: "Cast to 32-bit signed integer",
        detail: "int32(x [, mode])  — range −2^31..2^31−1. See int8 for cast rules." },
    HelpEntry { name: "int64", brief: "Cast to 64-bit signed integer",
        detail: "int64(x [, mode])  — range −2^63..2^63−1. See int8 for cast rules." },
    HelpEntry { name: "uint8", brief: "Cast to 8-bit unsigned integer",
        detail: "uint8(x [, mode])  — range 0..255. Negatives saturate to 0 (default)." },
    HelpEntry { name: "uint16", brief: "Cast to 16-bit unsigned integer",
        detail: "uint16(x [, mode])  — range 0..65535. See int8 for cast rules." },
    HelpEntry { name: "uint32", brief: "Cast to 32-bit unsigned integer",
        detail: "uint32(x [, mode])  — range 0..2^32−1. See int8 for cast rules." },
    HelpEntry { name: "uint64", brief: "Cast to 64-bit unsigned integer",
        detail: "uint64(x [, mode])  — range 0..2^64−1 (full range). See int8." },
    HelpEntry { name: "class", brief: "Type/class name of a value",
        detail: "class(x)  — returns e.g. \"int32\", \"double\", \"complex\", \"string\".\n  class(int32(5)) → \"int32\"." },
    HelpEntry { name: "cast", brief: "Convert to a named class",
        detail: "cast(x, \"int32\")  — convert to any integer class or \"double\".\n  Integer targets round half away from zero and saturate." },
    HelpEntry { name: "intmax", brief: "Largest value of an integer class",
        detail: "intmax([\"int32\"])  — default int32. intmax(\"int8\") → 127 (int8)." },
    HelpEntry { name: "intmin", brief: "Smallest value of an integer class",
        detail: "intmin([\"int32\"])  — default int32. intmin(\"int8\") → -128 (int8)." },
    HelpEntry { name: "isinteger", brief: "True for an integer-class value",
        detail: "isinteger(x)  — true only for int8…uint64 values (not doubles).\n  isinteger(int8(3)) → true; isinteger(3) → false." },
    HelpEntry { name: "isa", brief: "Class or category membership test",
        detail: "isa(x, \"class\")  — categories: \"numeric\", \"integer\", \"float\";\n  otherwise an exact class-name match. isa(int32(1), \"integer\") → true." },
    HelpEntry { name: "double", brief: "Convert to double precision",
        detail: "double(x)  — integers → real; bool → 0/1; reals/complex unchanged.\n  double(int16(1000)) → 1000. Note: int + double arithmetic already\n  yields a double." },
...
pub static CATEGORIES: &[CategoryRow] = &[
    ...
    CategoryRow { toolbox: "language", subcategory: "Cell arrays",
        names: &["iscell"] },
    CategoryRow { toolbox: "language", subcategory: "Integer types",
        names: &["int8", "int16", "int32", "int64", "uint8", "uint16", "uint32", "uint64",
                 "class", "cast", "intmax", "intmin", "isinteger", "isa", "double"] },
