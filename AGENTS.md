...
| Notebook GitHub-safe math | `crates/rustlab-notebook/src/markdown_safe.rs` | Complete — `--format markdown` (without `--obsidian`) auto-rewrites math-span tokens that GitHub's CommonMark passes corrupt. Two strategies: (1) emphasis-pairing fix — `^*`/`_*` → `^{\ast}`/`_{\ast}` (braced LaTeX synonym, no `*` left for emphasis to pair); (2) backslash-escape fix — `\,` `\;` `\:` `\!` `\|` → `\\,` `\\;` `\\:` `\\!` `\\|` (CommonMark turns `\\` → `\`, KaTeX gets the original). Earlier alpha-synonym approach (`\thinspace`, `\thickspace`, etc.) reverted: `\thickspace`, `\medspace`, `\negthinspace` are unreliable in GitHub's KaTeX configuration; doubling the backslash is an exact round-trip. Code spans, fenced code blocks, prose emphasis untouched. HTML and Obsidian markdown skip the rewrite. See `docs/notebooks.md` § "Markdown (`--format markdown`)". |
