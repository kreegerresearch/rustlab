...
| `eigsys` | `[V, D] = eigsys(M)` | Dense full eigendecomposition. `V` is the `n×n` eigenvector matrix (one column per eigenvalue) and `D` is the length-`n` eigenvalue vector. Algorithm: same Hessenberg-reduce + shifted-QR pipeline as `eig` for the eigenvalues, then shifted inverse iteration on `M` for each eigenvector. Defective matrices (repeated eigenvalues without enough independent eigenvectors) raise a runtime error — fall back to `eig(M)` when only eigenvalues are needed. |
