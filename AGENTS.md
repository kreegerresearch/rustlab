...
| Notebook GitHub-safe math | `crates/rustlab-notebook/src/markdown_safe.rs` | Complete — `--format markdown` (without `--obsidian`) auto-rewrites math-span tokens that GitHub's CommonMark passes corrupt: `^*`/`_*` → `^{\ast}`/`_{\ast}`, `\|` → `\Vert{}`, `\,` → `\thinspace`, `\;` → `\thickspace`, `\:` → `\medspace`, `\!` → `\negthinspace`. Replacements render identically under KaTeX. Code spans, fenced code blocks, prose emphasis untouched. HTML and Obsidian markdown skip the rewrite (their pipelines don't have the bug). See `docs/notebooks.md` § "Markdown (`--format markdown`)". |
