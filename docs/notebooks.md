...
| `W002` | warning | `<details>` / `</details>` tag counts don't balance (ignores fenced blocks and inline code spans). | no |
...
| `W005` | warning | Inline `$...$` math opener that does not close on the same line (renderer is same-line only; split delimiters leak raw TeX). | no |
...
**Heading anchors** — every h1–h3 automatically gets a GitHub-style slug
id from its text, so fragments written the natural way just work:

```markdown
## Section Two          → id="section-two"

[details](other.md#section-two)   ← resolves, no annotation needed
```
...
survives rewording the heading — an explicit id always wins over the
generated slug:
...
otherwise — the same ids cross-notebook fragments resolve to.
