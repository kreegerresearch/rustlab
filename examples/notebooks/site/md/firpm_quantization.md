...
```text
Peak stopband (float):    -56.18 dB
Peak stopband (naïve Q):  -38.66 dB
Peak stopband (firpmq):   -45.99 dB

firpmq improves stopband by 7.3 dB over naïve quantization
```

**Legend:** blue = `firpm` (float), red = naïve 10-bit quantization,
green = `firpmq` 10-bit optimized.
